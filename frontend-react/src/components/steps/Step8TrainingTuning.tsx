  ...
  const handleStartTraining = async () => {
    ...
    try {
      await runHyperparameterTuning({
        models: modelSelectionResult.selected_models,
      ...
      // Mock results for now - replace with actual API call results
      const mockResults: ModelTrainingResult[] = modelSelectionResult.selected_models.map(model => ({
        model_name: model,
        best_score: 0.85 + Math.random() * 0.1,
        best_params: { 
          n_estimators: Math.floor(Math.random() * 200) + 100,
          max_depth: Math.floor(Math.random() * 10) + 3,
          learning_rate: Math.round((0.01 + Math.random() * 0.2) * 100) / 100
        },
        cv_scores: Array.from({length: cvFolds}, () => 0.8 + Math.random() * 0.15),
              ...
              <section className="bg-card border border-border rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-primary">{progress.best_score.toFixed(4)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Elapsed</p>
                    <p className="text-sm font-medium">{Math.round(progress.elapsed_time / 60)}m</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">ETA</p>
                    <p className="text-sm font-medium">{Math.round(progress.eta / 60)}m</p>
                  </div>
                </div>
                ...
                <div className="w-full bg-muted/50 rounded-full h-2 mb-2">
                  <div
                    ...
                    style={{ width: `${(progress.current_trial / progress.total_trials) * 100}%` }}
                  />
                </div>
                <p className="text-xs text-muted-foreground">
